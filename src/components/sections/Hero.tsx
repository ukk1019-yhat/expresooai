      ...
      <div className="max-w-7xl mx-auto px-6 w-full grid grid-cols-1 lg:grid-cols-2 gap-16 lg:gap-8 items-center z-10">
        ...
        >
          <div className="inline-flex items-center gap-2 px-3 py-1.5 rounded-full bg-white/5 border border-[#d4c3ab] w-fit">
            ...
            <span className="text-xs font-medium text-[#5c4433] uppercase tracking-wider">Enterprise Behavioral Intelligence</span>
          </div>
          
          <h1 className="text-5xl lg:text-7xl font-bold tracking-tight text-[#2c1e16] leading-[1.1]">
            AI Training for <br />
          ...
          <p className="text-lg text-[#6e5646] max-w-xl leading-relaxed">
          ...
          <div className="flex flex-wrap items-center gap-4 mt-4">
            ...
            <a 
              ...
              className="inline-flex items-center justify-center gap-2 bg-transparent border border-[#c4b094] text-[#2c1e16] px-6 py-3.5 rounded-lg font-semibold hover:bg-[#e8decb] transition-colors"
            >
          ...
          <div className="glass-card p-6 relative z-10 shadow-2xl shadow-black/50">
            <div className="flex items-center justify-between mb-6 pb-4 border-b border-[#d4c3ab]">
              <div className="flex items-center gap-3">
                <div className="w-10 h-10 rounded-full bg-gradient-to-br from-zinc-800 to-zinc-900 border border-[#d4c3ab] flex items-center justify-center">
                  <BrainCircuit size={20} className="text-[#8b5a2b]" />
                </div>
                <div>
                  <h3 className="text-sm font-semibold text-[#2c1e16]">Simulation Session</h3>
                  <p className="text-xs text-[#6e5646]">Enterprise Deal · Round 3</p>
                </div>
              </div>
            ...
            <div className="space-y-4 mb-8">
              <div className="bg-white/5 p-4 rounded-lg border border-[#e3d5c1]">
                <p className="text-xs text-[#826a57] mb-2 uppercase tracking-wide font-semibold">AI Buyer Persona</p>
                <p className="text-sm text-[#5c4433]">"Your competitors are offering the same platform at 30% less. Why should we pay a premium for your solution?"</p>
              </div>
              <div className="bg-[#8b5a2b]/10 p-4 rounded-lg border border-[#8b5a2b]/20 ml-8 relative">
                <div className="absolute -left-3 top-1/2 -translate-y-1/2 w-1 h-8 bg-[#8b5a2b] rounded-full" />
                <p className="text-xs text-[#8b5a2b] mb-2 uppercase tracking-wide font-semibold">Your Response</p>
                <p className="text-sm text-[#2c1e16]">"That's a fair question. The difference is in what you're not seeing—our infrastructure reduces ramp time by 60%, meaning ROI in week one."</p>
              </div>
            </div>
...
function MetricCard({ icon, title, value, trend }: { icon: React.ReactNode, title: string, value: number, trend: string }) {
  ...
  return (
    <div className="bg-[#f4ebd8]/50 p-4 rounded-lg border border-[#e3d5c1]">
      <div className="flex items-center gap-2 text-[#6e5646] mb-3">
        {icon}
        <span className="text-xs font-medium uppercase tracking-wider">{title}</span>
      </div>
      <div className="flex items-end justify-between">
        <span className="text-2xl font-bold text-[#2c1e16]">{value}<span className="text-sm text-[#826a57] font-normal">/100</span></span>
      ...
      <div className="w-full h-1 bg-white/5 mt-3 rounded-full overflow-hidden">
        <motion.div 
          ...
          className={`h-full ${value > 80 ? "bg-emerald-500" : "bg-[#8b5a2b]"}`} 
        />
      </div>
    </div>
  );
